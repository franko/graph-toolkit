...
template <typename T, typename ResourceManager>
class drawing_adapter : public drawing {
public:
    drawing_adapter(T* d): m_drawing(d) { ResourceManager::acquire(m_drawing); }
    ~drawing_adapter() { ResourceManager::dispose(m_drawing); }
